...
void	gen_attr(t_obj *obj)
{
	...
	obj->attr.albedo = vec3((obj->attr.color >> 16 & 0xff) / 255.,
	(obj->attr.color >> 8 & 0xff) / 255., (obj->attr.color & 0xff) / 255.);
...
{
	q->j = q->a * pow(v.x, 2) + q->b * pow(v.y, 2) + q->c * pow(v.z, 2)
	+ q->d * v.y * v.z + q->e * v.x * v.z + q->f * v.x * v.y - q->g * v.x
	- q->h * v.y - q->i * v.z + q->j;
